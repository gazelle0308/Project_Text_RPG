...
int Player::GetMaxHp() const{ return this->maxHp; }
int Player::GetMaxMp() const{ return this->maxMp; }
...
void Player::GetGroupStatus(int (&status)[4]) // 위치 임의 지정 0 = maxHp, 1 = maxMp, 2 = power, 3 = defence
{
...
void Player::LevelUpPlayer()
{
    ...
    this->GetGroupStatus(beforeStatus); // 위치 임의 지정 0 = maxHp, 1 = maxMp, 2 = power, 3 = defence
    ...
    while(this->exp >= this->maxExp)
    {
        ...
        this->maxHp = this->maxHp + 10 * (level - 1);
        this->maxMp = this->maxMp + 10 * (level - 1);
        this->hp = this->maxHp;
        this->mp = this->maxMp;
    ...
    after = this->level;
    this->GetGroupStatus(afterStatus); // 위치 임의 지정 0 = maxHp, 1 = maxMp, 2 = power, 3 = defence
...
void CheckStatus(int Status[])
{
    ...
    while(true)
    {
        ...
        if(IsOutRange(Status[0], 50, 100) || IsOutRange(Status[1], 50, 100))
        {
            ...
            std::cout << "HP and MP must be at min 50 max 100 " << std::endl;
        ...
        else if(IsOutRange(Status[2], 20, 40) || IsOutRange(Status[3], 20, 40))
        {
            ...
            std::cout << "Attack and Defense must be at min 20 max 40" << std::endl;
            std::cout << "Please re-enter your character's stats: ";
            std::cin >> Status[2] >> Status[3];
            CutLine();
            continue;
...
bool UpgradeStatus(Player& player, Inventory<Item>& item)
{
    ...
    Item* bottle_ = nullptr;

    HpPotion = &item.GetItem("HpPotion");
    MpPotion = &item.GetItem("MpPotion");
    bottle_ = &item.GetItem("Bottle");
        ...
        switch(choice)
        {
            ...
            case 1:
                if (HpPotion->GetCount() > 0)
                {
                    ...
                    bottle_->SetCount(bottle_->GetCount() + 1);
                    StartLine();
...
bool IsOutRange(int value, int min, int max){ return value < min || value > max; }